...
export async function POST(request: Request) {
  try {
    ...
    if (!finalApiKey) {
      return NextResponse.json({ valid: false, error: 'IA ṇo configurada via painel ou .env' }, { status: 400 })
    }
    ...
    if (!res.ok) {
      ...
      const statusText = res.status === 429
        ? 'Erro 429 (Limite de requisi̵̤es / Cota excedida no provedor OpenCode)'
        : res.status === 404
        ? `Erro 404 (Rota ou modelo "${model}" ṇo encontrado em ${url})`
        : res.status === 401
        ? 'Erro 401 (API Key ṇo autorizada pelo provedor)'
    ...
    const message = error instanceof Error ? error.message : 'Erro ao validar conex̣o de IA'
