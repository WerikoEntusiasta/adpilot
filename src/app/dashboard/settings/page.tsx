...
function SettingsContent() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* OPENCODE / AI CONFIGURATION & DYNAMIC MODEL PICKER */}
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                OpenCode / OpenAI API Server
              </CardTitle>
              <CardDescription className="mt-1">
                Conecte seu servidor OpenCode ou fornecedor de IA compatível (`v1/models` integrado)
              </CardDescription>
            </div>
            <div>
              {aiStatus === 'valid' && <Badge variant="success" className="gap-1"><CheckCircle2 className="h-3 w-3" /> Conectado</Badge>}
              {aiStatus === 'invalid' && <Badge variant="destructive" className="gap-1"><XCircle className="h-3 w-3" /> Inválido</Badge>}
              {aiStatus === 'idle' && settings.aiApiKey && <Badge variant="secondary">Não testado</Badge>}
              {aiStatus === 'idle' && !settings.aiApiKey && <Badge variant="outline">Não configurado</Badge>}
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>Endpoint HTTP da IA (OpenCode / OpenAI Base URL)</Label>
            <Input
              value={settings.aiEndpoint}
              onChange={e => settings.setAiConfig({ aiEndpoint: e.target.value })}
              placeholder="Ex: http://localhost:8080/v1 ou https://api.openai.com/v1"
            />
          </div>

          <div className="space-y-2">
            <Label>API Key do OpenCode / Servidor</Label>
            <Input
              type={showTokens ? 'text' : 'password'}
              value={settings.aiApiKey}
              onChange={e => settings.setAiConfig({ aiApiKey: e.target.value })}
              placeholder="sk-... (ou insira sua chave do OpenCode)"
            />
          </div>

          {/* DYNAMIC MODEL SELECTOR FETCHED FROM OPENCODE */}
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <Label className="flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-purple-400" />
                Modelo Selecionado (OpenCode Models)
              </Label>
              <Button
                variant="ghost"
                size="sm"
                onClick={handleFetchAiModels}
                disabled={isLoadingModels || !settings.aiEndpoint}
                className="h-7 text-xs"
                title="Passe o mouse ou clique para atualizar modelos disponíveis no OpenCode"
                onMouseEnter={() => {
                  if (aiModels.length === 0 && !isLoadingModels) handleFetchAiModels()
                }}
              >
                <RefreshCw className={`h-3 w-3 mr-1 ${isLoadingModels ? 'animate-spin' : ''}`} />
                Atualizar Modelos (v1/models)
              </Button>
            </div>

            {aiModels.length > 0 ? (
              <Select value={settings.aiModel} onValueChange={v => settings.setAiConfig({ aiModel: v })}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="Selecione o modelo do OpenCode" />
                </SelectTrigger>
                <SelectContent>
                  {aiModels.map((m) => (
              ...
              <div className="space-y-2">
                <Input
                  ...
                  onChange={e => settings.setAiConfig({ aiModel: e.target.value })}
                  placeholder="Ex: opencode-zen, opencode-go, gpt-4o-mini"
                />
              </div>
            )}
            ...
            {modelsError && (
              <p className="text-xs text-amber-400">
                ⚠️ Aviso ao consultar modelos: {modelsError}
              </p>
            )}
          </div>
          ...
          {aiErrorDetails && (
            <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm space-y-1">
              <div className="flex items-center gap-2 font-bold">
                <AlertTriangle className="h-4 w-4 shrink-0" />
                <span>Diagnóstico do Servidor de IA:</span>
              </div>
              <p className="font-mono text-xs whitespace-pre-wrap">{aiErrorDetails}</p>
            </div>
          )}

          <div className="flex gap-2 pt-2">
            <Button variant="outline" onClick={handleValidateAi} disabled={isValidatingAi || !settings.aiEndpoint}>
              {isValidatingAi ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Shield className="h-4 w-4 mr-2 text-purple-400" />}
              {isValidatingAi ? 'Testando IA...' : 'Testar Conexão OpenCode IA'}
            </Button>
            <Button onClick={() => { setSavedAi(true); setTimeout(() => setSavedAi(false), 2000) }} disabled={!settings.aiEndpoint}>
              {savedAi ? <><CheckCircle2 className="h-4 w-4 mr-2" /> Salvo!</> : 'Salvar Configuração IA'}
            </Button>
          </div>
        </CardContent>
      </Card>
